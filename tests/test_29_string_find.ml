// ============================================================
// Test 29: string.find() メソッド (fix #54)
//   A. 基本検索（見つかる）
//   B. 見つからない場合 → None
//   C. 空文字列パターン → Some(0)
//   D. 先頭マッチ
...
//   H. find() 結果を match で分岐して計算
//
// カバレッジ観点:
//   C0  : find() を少なくとも 1 回実行
//   C1  :
//     A  : マッチあり（Some を返す）パス
//     B  : マッチなし（None を返す）パス
//     C  : 空文字列パターン（strstr が先頭ポインタを返す）パス
//   MC/DC:
//     F  : 複数出現時に最初の位置だけを返すことを確認
//     G  : 関数戻り値経由で Option<i32> を受け取るパス
//     H  : match で Some/None を分岐して整数演算するパス
// ============================================================

// ----------------------------------------------------------
// G. 関数戻り値として Option<i32> を返す
// ----------------------------------------------------------
